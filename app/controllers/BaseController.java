...
public class BaseController extends Controller
{
    ...
    public Result index()
    {
        ...
        List<MemberDetail> members = jpaApi.em().createNativeQuery("SELECT m.member_id as id, m.first_name as firstName, m.last_name as lastName, m.email, m.phone, ch.name as chapter, m.date_joined as dateJoined, m.volunteer, co.name as company, jt.name as jobTitle FROM member m  JOIN company co ON m.company_id = co.company_id  JOIN chapter ch ON m.chapter_id = ch.chapter_id JOIN job_title jt ON m.job_title_id = jt.job_title_id ORDER BY lastName", MemberDetail.class).getResultList();
